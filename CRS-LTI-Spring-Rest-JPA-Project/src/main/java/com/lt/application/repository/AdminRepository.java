 ...
 * 
 * import com.lt.application.bean.Course; import
 * com.lt.application.constant.SQLConstant;
 * 
 *//**
	 * @author Jeaswanth
	 *
	 */
/*
 * @Repository public interface AdminRepository extends JpaRepository<T, ID>{
 * 
...
/*
 * 
 * public JSONObject generateReportCard(String studentId) { String sql =
 * "select * from studentcourse sc inner join course c on c.course_id=sc.course_id inner join student s on s.id=sc.student_id where sc.student_id=?"
 * ;
 * 
 * HashMap<String, JSONObject> reportCard = new HashMap<>(); JSONArray jsa = new
 * JSONArray(); JSONObject jso = new JSONObject(); List<JSONObject> report =
 * jdbcTemplate.query(sql, new Object[] { studentId }, new
 ...
 * 
 * jso.put("studentId", rs.getString("id")); jso.put("studentName",
 * rs.getString("name"));
 * 
 * } JSONObject courseObj = new JSONObject(); courseObj.put("courseId",
 * rs.getString("course_id")); courseObj.put("courseName",
 * rs.getString("course_name")); courseObj.put("grade", rs.getString("grade"));
 * jsa.put(courseObj);
 * 
 * return null; } });
 * 
 ...
 * 
 * jdbcTemplate.update(SQLConstant.ADD_COURSE_DETAILS, course.getId(),
 * course.getName(), course.getNoOfStudents(), course.getPaymentFee());
 * 
 * 
 * return "";
 * 
 ...
 *//**
	 ...
	 */
/*
 * public String addprofessor(String id, String name, String password) { // TODO
